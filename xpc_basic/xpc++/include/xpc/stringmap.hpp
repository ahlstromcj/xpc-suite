#if ! defined XPC_STRINGMAP_HPP
#define XPC_STRINGMAP_HPP
...
XPC_REVISION_DECL(stringmap)           /* show_stringmap_info()               */

#if XPC_HAVE_STDIO_H
#include <stdio.h>					      /* fprintf() [gasp!!]                  */
#endif
 ...
 *
 *    This implementation is primitive; we prefer the for_each style of
 *    implementation.  But we may want to do performance testing of show()
 *    versus show_primitive() to see if for_each() really saves anything.
 *
 * \param tag
 *    Identifies the object in the human-readable output.
 *
...
)
{
   ...
   typename stringmap<VALUETYPE>::const_iterator ci;
   for (ci = container.begin(); ci != container.end(); ci++)
   {
      VALUETYPE value = ci->second;
      show(value);
   }
}
 *
