 ...
 *       -  <i>Comment.  Lines whose first non-white-space character is in
 *          the set <code> #;!'!" </code> are ignored.
...
#include <xpc/initree.hpp>             /* the functions in this module        */
 ...
 *------------------------------------------------------------------------*//**
 *
 *    Creates an named, but empty, initree.  The name can be added or
 ...
 *
 * \return
 *    Return's the name of the newly-created section.  If this name is
...
)
{
   ...
   if (result)
   {
      ...
      if (result)
      {
         ...
         std::string::size_type pequals = pvalue;
         if (pvalue == std::string::npos)
            no_value = true;

         if (! no_value)
         {
            std::string::size_type poptionend = pvalue;  // point to "="
         ...
         /*
          * We now know the disposition of the value (do we have one, what is
          * it)?  Now to peel out the option.  If there is no value, the end of
          * the string or start of spaces ends the option.  If there is a
          * value, the "=" can also end the option.  The pbegin index points
...
void
show (const std::string & tag, const initree & container)
{
   fprintf
   (
      stdout,
   ...
   initree::const_iterator ci;
   for (ci = container.begin(); ci != container.end(); ci++)
   {
      show("initree::Section", ci->second);
   }
}
